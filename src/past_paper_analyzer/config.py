...
def check_config():
    ...
    if not CL_AUTH_COOKIE:
        print("Warning: CL_AUTH_COOKIE environment variable not set in .env file. Downloading will fail.", file=sys.stderr)
    ...
    try:
        ...
        print(f"Error creating downloads directory '{download_dir}': {e}", file=sys.stderr)
        config_ok = False
