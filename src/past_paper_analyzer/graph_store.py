...
def load_graph(path: str = config.GRAPH_DATA_PATH) -> nx.DiGraph:
    ...
    if os.path.exists(path):
        ...
        try:
            ...
            print(f"Error loading graph from {path}: {e}. Creating a new graph.", file=sys.stderr)
...
def save_graph(graph: nx.DiGraph, path: str = config.GRAPH_DATA_PATH):
    ...
    print(f"Saving graph with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges to {path}")
    try:
        # Ensure the directory exists
        os.makedirs(os.path.dirname(path), exist_ok=True)
        nx.write_graphml(graph, path, infer_numeric_types=True) # infer_numeric_types helps preserve types
...
def add_question(graph: nx.DiGraph, paper_node_id: str, question_number: str, course_module: str = None) -> str:
...
def add_concept(graph: nx.DiGraph, concept_name: str, definition: str = None) -> str:
    ...
    if not canonical_name: # Handle empty or invalid names
        print(f"Warning: Skipping invalid concept name '{original_name}'", file=sys.stderr)
        return None
    ...
    graph.add_node(
        ...
        definition=definition if definition else graph.nodes.get(node_id, {}).get('definition', "No definition provided"), # Keep old def if new one is None
...
def link_question_to_concept(graph: nx.DiGraph, question_node_id: str, concept_node_id: str):
    """Adds a MENTIONS relationship from a Question to a Concept."""
    if question_node_id is None or concept_node_id is None:
        print(f"Warning: Skipping link due to invalid node ID (Q: {question_node_id}, C: {concept_node_id})", file=sys.stderr)
        return
    if question_node_id not in graph:
        print(f"Warning: Question node '{question_node_id}' not found. Cannot link concept.", file=sys.stderr)
        return
    if concept_node_id not in graph:
         print(f"Warning: Concept node '{concept_node_id}' not found. Cannot link from question.", file=sys.stderr)
         return
